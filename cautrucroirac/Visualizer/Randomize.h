...
namespace Randomize {
	...
	class Float : public Randomizer<float> {
	private:
		float _min;
		float _max;
		int _decimalPlaces;
	...
	public:
		Float(float min = 0, float max = 1, int decimalPlaces = 2) {
			_min = min;
			_max = max;
		...
		float create() {
			std::uniform_real_distribution<float> dist6(_min, _max);
			float randomFloat = dist6(rng);
			float factor = (float)pow(10, _decimalPlaces);
			return round(randomFloat * factor) / factor;
		}
	};
	...
	public:
		...
		string create() {
			string text = "";
			Integer randomizer(0, (int)_letters.length() - 1);
			...
			for (int i = 0; i < _N; i++) {
				for (int j = 0; j < _N; j++) {
					if (i == j) {
						...
						G[i * MAX + j] = ratioRandomizer.create() < _ratio ? _weighted ? (*_randomizer).create() : 1 : 0;
					}
					else {
						G[i * MAX + j] = G[j * MAX + i];
					}
